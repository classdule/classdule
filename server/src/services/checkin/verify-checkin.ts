...
export class VerifyCheckin {
    ...
    async do({checkinId, verify}: Request){
        ...
        const targetClassroom = await this.classroomRepository.findById(targetCheckin.classroomId);
        if(!targetClassroom){
            throw new Error(`Classroom not found with id ${targetCheckin.classroomId}`);
        }
